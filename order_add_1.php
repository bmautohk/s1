...
//check sale order no, same order no
	...
	if (isset($_POST['sale_ref_a']))
	{
	 ...
	 //insert record
	 if ($_POST['sale_ship_fee']!='') {$sale_ship_fee = $_POST['sale_ship_fee'];} else {$sale_ship_fee = 0;}
	 if ($_POST['sale_discount']!='') {$sale_discount = $_POST['sale_discount'];} else {$sale_discount = 0;}
	 if ($_POST['sale_tax']!='') {$sale_tax = $_POST['sale_tax'];} else {$sale_tax = 0;}
	...
	$sql = "INSERT INTO ben_sale SET
			...
			sale_yahoo_id = '".$_POST['sale_yahoo_id']."',
		 	sale_dat = curdate(), 
  ...
  <TR>
    <TD vAlign=top bgColor=#eefafc colSpan=6 height="100%">
      <TABLE cellSpacing=0 cellPadding=0 width="100%">
        <TBODY>
        <TR>
          <TD bgColor=#beddeb><TABLE width="315" cellPadding=10>
            <TBODY>
              <TR>
                <TD><span class="cat cat"><a href="order_add.php" class="big">Add New sales</a> </span></TD>
                <TD>&nbsp;</TD>
              </TR>
            </TBODY>
          </TABLE></TD>
