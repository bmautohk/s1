...
// add new order
if (isset($_POST['importYahoo']) || isset($_POST['importYahooShopping']) || isset($_POST['importRakuten']) || isset($_POST['importRakuten2018'])  ) {
	$import = true;
	$isValid = true;
	...
	if ($isValid) {
		...
		} else if (isset($_POST['importRakuten2018'])) {
			$import_success = importRakuten2018($db, $file_name, $actual_file_name, $salesGroup);
		}
	}
	...
	for ($k=1;$k<=$prod_n;$k++)
	{
		...
		$sqla = "INSERT INTO ben_sale_prod SET
			sprod_ref = '$sale_ref',
			sprod_id =  upper('".trim($_POST[$sprod_id])."'),
...
function importRakuten2018($db, $file_name, $actual_file_name, $salesGroup) {
	global $error_message;
	...
	foreach ($ws->getRowIterator() as $row) {
		...
		$order['order_date'] = date('Y-m-d');
		$order['sale_group'] = $salesGroup;
		$order['sale_name'] = convert($ws->getCell("BL".$rowNo)->getValue()
								.$ws->getCell("BM".$rowNo)->getValue());
								//.$ws->getCell("AA".$rowNo)->getValue()
								//.$ws->getCell("AB".$rowNo)->getValue());
		$order['sale_ship_fee'] = $ws->getCell("AB".$rowNo)->getValue();
	
		$order['sale_email'] = $ws->getCell("AV".$rowNo)->getValue();
		$order['sale_yahoo_id'] = '';
		$order['sale_chk_ref'] = 0;
		$order['sale_discount'] = $ws->getCell("AH".$rowNo)->getValue();
		$order['sale_tax'] = $ws->getCell("AA".$rowNo)->getValue()/$ws->getCell("Z".$rowNo)->getValue()*100;

	/** Product */
		$product['sale_ref'] = $order['sale_ref'];
		$product['sprod_name'] = convert($ws->getCell("BU".$rowNo)->getValue());
		//$product['sprod_price'] = $ws->getCell("H".$rowNo)->getValue();
		$product['sprod_price'] = $ws->getCell("BX".$rowNo)->getValue();
		$product['sprod_unit'] = $ws->getCell("BY".$rowNo)->getValue();
		$product['sprod_material'] = convert($ws->getCell("DD".$rowNo)->getValue());
		
		$colour = $ws->getCell("CC".$rowNo)->getValue();
		if (!empty($colour)) {
			$product['sprod_colour'] = convert(substr($colour, 4));
		}
		...
		//$select = $ws->getCell("I".$rowNo)->getValue();
		//if (empty($select)) {
			$product['sprod_id'] = $ws->getCell("BV".$rowNo)->getValue();
		//} else {
	//		$pos1 = strrpos($select, '「');
	//		$pos2 = strrpos($select, '」');
	//		$product['sprod_id'] = substr($select, $pos1 + 3, $pos2 - $pos1 - 3);
	...
	/** Debt */
		...
		$addr = $ws->getCell("BI".$rowNo)->getValue()
						.$ws->getCell("BJ".$rowNo)->getValue()
						.$ws->getCell("BK".$rowNo)->getValue();
		...
		$debt['debt_email'] = $ws->getCell("AV".$rowNo)->getValue();
		$debt['debt_post_co'] = $ws->getCell("BG".$rowNo)->getValue().'-'
								.$ws->getCell("BH".$rowNo)->getValue();
		$debt['debt_tel'] = $ws->getCell("BP".$rowNo)->getValue().'-'
							.$ws->getCell("BQ".$rowNo)->getValue().'-'
							.$ws->getCell("BR".$rowNo)->getValue();
		$debt['debt_mobile'] = '';
		$debt['debt_bank'] = '';
		$debt['debt_pay_method'] = 'Bank in';
		$debt['debt_shipping_method'] = 'Air Mail';
		...
		// Remarks
		$remark = $ws->getCell("CW".$rowNo)->getValue();
	...
	//20190829	if ($requiredCost != NULL && $requiredCost != '') {
		...
		//deliver date 20180807 -- 20180827 fix problem import for bal table
			...
			//$dateArr=explode('/',$ws->getCell("CR".$rowNo)->getValue());
			//$balance['bal_delivery_date'] =  $dateArr[2].'-'.$dateArr[1].'-'.$dateArr[0];
			//echo $balance['bal_delivery_date'];
			
			
			
			//[配送日時指定:]19～21時[注文フォーム:]
			//$delivery = $ws->getCell("CW".$rowNo)->getValue();
			//$start_idx = mb_strpos($delivery, ']', 0, 'UTF-8');
			//$end_idx = mb_strpos($delivery, '[', 3, 'UTF-8');
			//$delivery_time = convert(mb_substr($delivery, $start_idx + 1, $end_idx - $start_idx - 1, 'UTF-8'));
			//$balance['bal_delivery_time_optaion_id'] = NULL;
			...
			if ($type == '代金引換') {
				...
				$debt['debt_remark'] = $debt['debt_remark'].'\n'.convert('代引 : '.$ws->getCell("AC".$rowNo)->getValue().'円');
			} else {
				$balance['bal_ship_type'] = '';
			}
	//	}
