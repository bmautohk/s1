...
if ($_POST['event'] == 'save') {
	...
	for ($k=1;$k<=$prod_n;$k++)
	{
		...
		if ($prod_cd != '' and $prod_stock_prod_id == '') {
			...
			$sqla = "INSERT INTO temp_instock_product SET
					instock_id = '$stock_id',
					product_cd = '$prod_cd',
			...
			$sqla = "UPDATE temp_instock_product SET
					product_cd = '$prod_cd',
					product_name = '$prod_name',
					remark = '$prod_remark',
					colour = '$prod_colour',
...
if ($stock_id != '') {
	...
	// Retrieve stock
	$sql = "SELECT * FROM temp_instock WHERE id = $stock_id ";
	...
	$sql = "SELECT * FROM temp_instock_product WHERE instock_id = $stock_id AND sts <> 'X' ";
