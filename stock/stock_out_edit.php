...
<TD vAlign=top bgColor=#eefafc>
		    ...
		    <form name="form1" method="POST" action="<?= $_SERVER['PHP_SELF']; ?>?page=<?=$page?>&subpage=<?=$subpage?>" >
                <input type="hidden" name="event" value="">
                   ...
                   <br /><br />
                    Number of Product
                   <select name="prod_n" onChange=javascript:location.href='index.php?page=stock&subpage=out_edit&stock_id=<?=$stock_id ?>&prod_n='+this.options[this.selectedIndex].value>
                    <? 
						for ($i=1;$i<=80;$i++)
						{
							if ($prod_n==$i)
