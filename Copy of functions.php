...
function getorder_list_by_no($sale_ref,$check)
	...
	{
	$result = mysql_query("SELECT * FROM ben_sale, ben_debt where sale_ref = debt_ref and sale_name like '%$sale_ref%'" ,$db) or die (mysql_error()."<br />Couldn't execute query: $query");
	} 
	$num_results=mysql_num_rows($result);
	//table echo 
	echo "<table width=\"850\" border=\"1\" cellspacing=\"0\" cellpadding=\"0\">";
...
function getreturn_list($sale_ref)
	...
	$sub_total=0;
	$num_results=mysql_num_rows($result);
	$email_text = $email_text . "はじめまして　高元　伸康と申します<br>今回は以下商品を落札いただきましてありがとうございます。<br><br>";
	...
	$email_text = $email_text . "落札金額：".number_format($sprod_sub,2,'.','')."円+";
	...
	$email_text = $email_text . "送料 ".number_format($sale_ship_fee,2,'.','')."円";
	
	$email_text = $email_text . "= ".number_format($total_temp_ship_sub,2,'.','')."円<br><br>";
	...
	$email_text = $email_text . "よろしくおねがいします<br><br>";
	$email_text = $email_text . "  高元　伸康 <br>";
