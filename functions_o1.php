...
//remark
	if (getremark_data($sale_ref))	
	{$remark = getremark_data($sale_ref);
...
echo "</table><br><br>";
	...
	$db=connectDatabase();
	mysql_select_db("SUPERIOR_AUTOPARTS",$db);
	$result = mysql_query("SELECT * FROM ben_bal, ben_sale where bal_ref not in (select check_ref from ben_check) and sale_ref=bal_ref order by bal_dat asc" ,$db) or die (mysql_error()."<br />Couldn't execute query: $query");
	
	$num_results=mysql_num_rows($result);
	//table echo 
	echo "<table width=\"300\" border=\"1\" cellspacing=\"0\" cellpadding=\"0\">";
	echo "<tr align=\"right\" valign=\"top\"><td >Order No.</td><td >Sale Group</td><td >Payment Date</td><td >Add Shipping No.</td>\n";
...
*/		
	...
	echo "<tr align=\"right\" valign=\"top\"> <td><a href='order_edit.php?sale_ref=$bal_ref'>".$bal_ref."</a></td><td>".$sale_group."</td><td>".$bal_dat."</td><td><a href='shipping.php?sale_ref=$bal_ref'>".$bal_ref."</a></td></tr>\n";
	}
	//end loop
	echo "</table>";
	}
...
//--------------------------------------------------------------------------	
	...
	echo "<tr align=\"right\" valign=\"top\"><td >Order No.</td><td >Sale Group</td><td >Shipping Date</td><td >Tracking No.</td><td >Detail</td>\n";
	...
	echo "<tr align=\"right\" valign=\"top\"> <td><a href='order_edit.php?sale_ref=$check_ref'>".$check_ref."</a></td><td>".$sale_group."</td><td>".$check_date."</td><td>".$check_shipping."</td><td ><a target='_blank' href='shipping.php?sale_ref=$check_ref'>Click</a></td></tr>\n";
	}
	//end loop
	echo "</table>";
	}
...
function getorder_list_by_filter($sale_ref, $sale_name,$sale_email,$sale_yahoo_id,$date_start,$date_end,$min_m,$max_m,$debt_cust_address1,$debt_cust_address2,$debt_post_co,$total_m)

	{
	$db=connectDatabase();
	mysql_select_db("SUPERIOR_AUTOPARTS",$db);
	
	$sql = "select *, (sum(sprod_unit*sprod_price) ) as sprod_total from ben_sale as s, ben_sale_prod as s1 where s.sale_ref = s1.sprod_ref and sale_date between '$date_start' and '$date_end' ";
	if ($debt_cust_address1!='' or $debt_cust_address2!='' or $debt_post_co!='')
	{$sql = "select *, (sum(sprod_unit*sprod_price) ) as sprod_total from ben_sale as s, ben_sale_prod as s1, ben_debt as d where d.debt_ref= s.sale_ref and s.sale_ref = s1.sprod_ref and sale_date between '$date_start' and '$date_end' ";
	}
	...
	if ($total_m!=''){$sql = $sql."having sprod_total = '$total_m'"; } 
	}
	...
	function getgroup_select($sel_group)
{
	...
	if ($sel_group==$group_id)
	echo "<option selected value=\"$group_name\">$group_name</option>\n";
	else
