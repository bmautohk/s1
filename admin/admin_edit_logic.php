...
if (@$_POST[del_user] != "")
{
	...
	if ($num != 0)
	{
		...
		$trash_user = "INSERT INTO trash VALUES
	...
	if (@$_POST[modify] == "password")
		{
			...
			$_POST[modify] = password('$_POST[change]' )
			WHERE username = '$_POST[username]'";
			$result = @mysql_query($sql,$connection) or die(mysql_error());
			@$msg .= "The $_POST[modify] for $_POST[username] has been changed to $_POST[change].<br>";
		}else{
			$sql = "UPDATE $table_name SET 
			$_POST[modify] = '$_POST[change]' 
			WHERE username = '$_POST[username]'";
			$result = @mysql_query($sql,$connection) or die(mysql_error());
			@$msg .= "The $_POST[modify] for $_POST[username] has been changed to $_POST[change].<br>";
		}
...
if (@$_POST[ban_user] != "")
{
		...
		$ban = "INSERT INTO banned VALUES ('$_POST[ban_user]', 'user')";
...
if ($ip_addr != "...")
{
		$ban_ip = "INSERT INTO banned VALUES ('$ip_addr', 'ip')";
...
if (@$_POST[lift_ip_ban] != "")
{
		...
		$lift_ip = "DELETE FROM banned WHERE no_access = '$_POST[lift_ip_ban]'";
...
if (@$_POST[restore] != "")
{
	...
	if ($num != 0)
	{
		...
		$r_user = "INSERT INTO $table_name VALUES
...
if (@$_POST[amt_time] != "" &&  @$_POST[incr_time] != "")
{
	...
	while ($xfer = mysql_fetch_object($resultp1))
	{
		...
		$xfer2 = "INSERT INTO trash VALUES ('$pfirst', ' $plast', '$puname', '$ppass', '$pgr1', '$pgr2', '$pgr3',
					'$ppchange', '$pe_mail', '$pre_direct', '$pver_d', '$plast_log', '$pdel_date')";
	...
	$msg .= "Accounts older than $amt_time $incr_time has been moved to the trash.<br>";
}
...
?>
<?include_once('admin_edit_js.php');?>